...
CommPacketV3::CommPacketV3(TCPSocketClient *comm)
{
    serial_device = comm;
...
CommPacketV3::~CommPacketV3()
{
    ...
    std::cout <<  "CommPacketV3::~CommPacketV3()" << std::endl;
}

ssize_t CommPacketV3::Init()
{
    int ret_thrd = PthreadCreate(&thread_tx_, NULL,&CommPacketV3::Serial_Comm_Send_Thread,this);
    if (ret_thrd != 0) 
    {
        // ROS_WARN_STREAM(__FUNCTION__ << ": " << "Thread creation failed");
        return -1;
    }
    ret_thrd = PthreadCreate(&thread_rx_, NULL,&CommPacketV3::Serial_Comm_Recv_Thread,this);
    if (ret_thrd != 0) 
    {
        // ROS_WARN_STREAM(__FUNCTION__ << ": " << "Thread creation failed");
        return -1;
...
uint32_t CommPacketV3::CRC32_Calculate(uint8_t * data, uint16_t len)
{
...
// ===================================================================================
void *CommPacketV3::Serial_Comm_Send_Thread(void)
{
...
// ===================================================================================
void *CommPacketV3::Serial_Comm_Recv_Thread(void)
{
    uint8_t data[1000];
    int     bytes_count = 0;
...
int CommPacketV3::Serial_Comm_Send_Data(uint8_t *data,uint16_t len)
{
    int ret = serial_device->Send(data,len); 
    return ret;
}

uint8_t *CommPacketV3::Serial_Comm_Recv_Data(uint8_t *data,uint16_t len)
{
    MSG_t  *msg = (MSG_t*)data;
#if 1
    while((uint8_t*)msg <= (data + len - sizeof(MSG_t) - 2))
    {
        if(msg->sync1 == 0x55 && msg->sync2 == 0xAA)
        {
            if(((uint8_t*)msg + msg->length + 5) <= (data + len))
            {
                uint16_t dtc1 = *(uint32_t*)(msg->data + msg->length -2);
                uint16_t dtc2 = CRC32_Calculate((uint8_t*)msg,msg->length - 2 + 5);
                if(dtc1 == dtc2)
                {
                    ...
                    {
                        begin->second.second(begin->second.first,msg->cmd,msg->data,msg->length - 2);
                    }
                    else
                    {
                        // ROS_WARN_STREAM("Serial_Comm_Recv_Data: message type " << hex << msg->type <<" not registered!");
                    }
                    msg = (MSG_t*)((uint8_t*)msg + msg->length + 4);
                }
                else 
                {
...
// ===================================================================================
ssize_t CommPacketV3::SendMessage(uint16_t msg_type,uint8_t * msg_data, size_t data_size)
{
    ...
    if(msg.first != NULL)
    {
        ((MSG_t*)msg.first)->sync1   = 0x55;
        ((MSG_t*)msg.first)->sync2   = 0xAA;
        ((MSG_t*)msg.first)->length  = data_size + sizeof(crc); //( size(data) + size(crc))
        ((MSG_t*)msg.first)->cmd     = msg_type;
        msg.second      = sizeof(MSG_t) + data_size + 2;
        memcpy(((MSG_t*)msg.first)->data,(uint8_t*)msg_data,data_size);
        crc = CRC32_Calculate(msg.first,msg.second - 2);
