#ifndef _LIDAR_COMM_PACKET_HPP_
#define _LIDAR_COMM_PACKET_HPP_
...
public:
    LidarCommPacket(TCPSocketClient *comm);
    ~LidarCommPacket();

    ssize_t Init();
    bool state();

    ssize_t GetPacket(uint8_t* &msg_data) 
    {
        msg_data = msg_data_;
        return msg_len_;
    };
