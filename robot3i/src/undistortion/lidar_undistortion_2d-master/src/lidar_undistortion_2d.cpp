...
class LidarMotionCalibrator {
   ...
   public:
    LidarMotionCalibrator(ros::NodeHandle node_handle) {
        ...
        nh_param.param<std::string>("scan_pub_topic", scan_pub_topic_,
                                    "/lidar_undistortion/scan");
        nh_param.param<bool>("enable_pub_pointcloud", enable_pub_pointcloud_,
                             false);
        nh_param.param<std::string>("point_cloud_pub_topic",
                                    pointcloud_pub_topic_,
                                    "/lidar_undistortion/pointcloud");
        nh_param.param<std::string>("lidar_frame", lidar_frame_, "laser_link");
        nh_param.param<std::string>("odom_frame", odom_frame_, "odom");
        nh_param.param<double>("lidar_scan_time_gain", lidar_scan_time_gain_,
                               1.0);

        scan_sub_ = nh_.subscribe(scan_sub_topic_, 10,
    ...
    void ScanCallBack(const sensor_msgs::LaserScanConstPtr& scan_msg) {
        ...
        endTime = startTime + ros::Duration(scan_msg->time_increment *
        ...
        for (int alpha = beamNum - 1; alpha >= 0; alpha--) {
            ...
            angles.push_back(scan_msg->angle_min +
                             scan_msg->angle_increment * alpha);
        }
        ...
        publishScan(scan_msg, ranges, angles, intensities, start_pose,
                    end_pose);
    }
    ...
    bool getLaserPose(tf::Stamped<tf::Pose>& odom_pose,
        ...
        try {
            ...
            ROS_ERROR(
                "LidarMotion: No Transform available Error looking up robot "
                "pose: %s\n",
                ex.what());
            return false;
        } catch (tf::ConnectivityException& ex) {
            ROS_ERROR(
                "LidarMotion: Connectivity Error looking up looking up robot "
                "pose: %s\n",
                ex.what());
            return false;
        } catch (tf::ExtrapolationException& ex) {
            ROS_ERROR(
                "LidarMotion: Extrapolation Error looking up looking up robot "
                "pose: %s\n",
                ex.what());
            return false;
        }
     ...
     */
    void Lidar_MotionCalibration(tf::Stamped<tf::Pose> frame_base_pose,
        ...
        for (int index = 0; index < beam_number; ++index) {
            ...
            tf::Quaternion cur_quaternion =
                start_quaternion.slerp(end_quaternion, index * beam_step);
            tf::Vector3 cur_point =
                start_point.lerp(end_point, index * beam_step);
            ...
            if (!tfFuzzyZero(lidar_range) && !std::isinf(lidar_range)) {
                ...
                tf::Vector3 corrected_lidar_point =
                    tf::Pose(base_quaternion, base_point).inverse() *
                    frame_cur_pose * lidar_point;

                ranges[startIndex + index] = hypot(
                    corrected_lidar_point.getX(), corrected_lidar_point.getY());
                angles[startIndex + index] = atan2(
                    corrected_lidar_point.getY(), corrected_lidar_point.getX());
            } else {  // 如果为0
                ...
                angles[startIndex + index] =
                    tfNormalizeAngle(tmp_angle - base_angle);
            }

            // ROS_INFO("\t%d\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t",
            // startIndex + index, base_point.getX(), base_point.getY(),
            // tf::getYaw(base_quaternion),
            //                     cur_point.getX(), cur_point.getY(),
            //                     tf::getYaw(cur_quaternion),
            // lidar_range, lidar_angle, ranges[startIndex + index],
            // angles[startIndex + index]);
        }
        // end of TODO
    }
    ...
    void Lidar_Calibration(std::vector<float>& ranges,
        ...
        double time_inc =
            (end_time - start_time) / beamNumber;  // 每束激光数据的时间间隔
        ...
        for (int i = 0; i < beamNumber; i++) {
            ...
            if (mid_time - start_time > interpolation_time_duration ||
                ...
                //得到起点和终点的位姿
                //终点的位姿
                if (!getLaserPose(frame_mid_pose,
                                  ros::Time(mid_time / 1000000.0), tf_)) {
    ...
    void publishScan(const sensor_msgs::LaserScanConstPtr& scan_msg,
        ...
        for (int alpha = ranges.size() - 1; alpha >= 0; --alpha) {
            double angle = (angles[alpha] < 0 || tfFuzzyZero(angles[alpha]))
                               ? angles[alpha] + 2 * M_PI
                               : angles[alpha];
            angle += publish_msg.angle_min;
            int index = (int)((angle - publish_msg.angle_min) /
                              publish_msg.angle_increment);
        ...
        // publish_msg.angle_max = scan_msg->angle_max;
        // publish_msg.angle_min = scan_msg->angle_min -
        // tf::getYaw(start_pose.getRotation()) +
        // tf::getYaw(end_pose.getRotation()); publish_msg.angle_increment =
        // (publish_msg.angle_max - publish_msg.angle_min) /
        // (float)(angles.size() - 1); for(int index = ranges.size() - 1; index
        // >= 0; --index) {
        //     publish_msg.ranges.push_back(ranges[index]);
        // }

        // if(fabs(- tf::getYaw(start_pose.getRotation()) +
        // tf::getYaw(end_pose.getRotation()) > 0.01)) {
        //     ROS_INFO("after- angle_min: %f, angle_max: %f, angle_increment:
        //     %f", publish_msg.angle_min, publish_msg.angle_max,
        //     publish_msg.angle_increment); ROS_INFO("orign- angle_min: %f,
        //     angle_max: %f, angle_increment: %f", scan_msg->angle_min,
        ...
        //         angle_beta = publish_msg.angle_min + beta *
        //         publish_msg.angle_increment;

        //         ROS_INFO("\t%d\t%f\t%f\t%f\t%f\t", alpha, range_alpha,
        //         angle_alpha, range_beta, angle_beta);
        //     }
        // }
